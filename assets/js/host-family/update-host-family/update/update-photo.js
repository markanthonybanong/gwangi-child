export default $(function(){
    ...
    function updateHostFamilyPhoto(photo){
        $.ajax({
            type: 'POST',
            url: myAjax.restURL + 'activeAupair/v1/updateHostFamilyPhoto',
            data: {
                photo: photo,
    ...
    uploadBtn.click( function (){
        ...
        const photo = $('.upload-photo-container #host-family-photo').prop('files')[0];
        ...
        if(photo === undefined){
            ...
            updateHostFamilyPhoto("null");
        } else {
              //array of array
            const formData = new FormData();
            formData.append('hostFamilyPhoto', photo);
            //to fetch new image every time
            formData.append('randomFiveDigits', Math.floor(Math.random() * 90000) + 10000);
            $.ajax({
                type: 'POST',
                url: myAjax.restURL + 'activeAupair/v1/uploadHostFamilyPhoto',
                data: formData,
                contentType: false,
                cache: false,
                processData: false,
                ...
                success: function(response){
                    if(response.success){
                        updateHostFamilyPhoto(response.data);
                    }
                },
                error: function (xhr, ajaxOptions, thrownError) {
                },
